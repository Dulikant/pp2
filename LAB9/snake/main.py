...
pygame.display.set_caption('Змейка от Duksss')
game_window = pygame.display.set_mode((window_x, window_y),flags=pygame.NOFRAME)
...
while True :
    ...
    # Проверяем, прошло ли уже 5 секунд с момента появления яблока
    if current_time - last_fruit_time >= 5000:  # 5000 миллисекунд = 5 секунд
        fruit_position = [random.randrange(1, (window_x // 10)) * 10,
                          random.randrange(1, (window_y // 10)) * 10]
        last_fruit_time = current_time  # Обновляем время появления последнего яблока
